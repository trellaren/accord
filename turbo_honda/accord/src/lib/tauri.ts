 ...
 *
 * Creates the server record locally (using the canonical server id from the
 * invite) if it does not already exist, then joins the server.
 */
export function acceptServerInvite(
  ...
  return invoke("accept_server_invite", {
    serverId,
    serverName,
    inviteCode,
    ownerPeerId,
 ...
 * ```
 * This string is generated by the server owner's {@link InviteCodeModal}.
 */
export function joinServerByAddress(joinString: string): Promise<ServerInfo> {
  return invoke("join_server_by_address", { joinString });
}
